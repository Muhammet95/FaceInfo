...
CREATE TABLE `client_requests` (
  ...
  `task_result` float DEFAULT NULL,
  `client_name` char(128) NOT NULL,
  `client_filename` char(255) NOT NULL,
  `client_mime_type` char(16) NOT NULL,
  `task_retry_id` varchar(64) DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `request_name_and_filename` (`client_name`,`client_filename`),
  UNIQUE KEY `request_retry_id` (`task_retry_id`)
...
-- Dump completed on 2023-05-17 21:26:14
